...
- **Cover/bootleg burial fix** (eval-driven, 2026-05-29): copycat uploads that embed the real artist in their TITLE (e.g. `"Blinding Lights The Weeknd (Piano Arrangement)"`) normalize to the full query and land in the genuine track's relevance band, then beat it on popularity. Two coupled fixes in `dedup.py`:
  ...
  - **Still open**: bootleg re-uploads with a *junk artist* subtitle (e.g. `"Blinding Lights The Weeknd" / "Pancadão GD Som"`) carry no demotable marker — they rank below the genuine now but still clutter the band. The discriminating signal (genuine carries the queried artist in `subtitle`, bootleg in `title`) is the next lever.
