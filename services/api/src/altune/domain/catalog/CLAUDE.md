...
- **Track** (`track.py`) — aggregate root. Fields: `id: TrackId`, `user_id: UserId`, `title`, `artist`, `album?`, `duration_seconds?`, `added_at`, `artwork_url?`, `acquisition_status: AcquisitionStatus = PENDING`. Invariants enforced at construction: non-empty title + artist, non-negative duration. Identity + equality/hash are by `id` only. `artwork_url` + `acquisition_status` were added by the `view-result-detail` spec.
- **TrackId** (`track_id.py`) — UUID identity wrapper.
- **AcquisitionStatus** (`acquisition_status.py`) — audio-acquisition lifecycle enum. v1 has a single member `PENDING = "pending"` ("saved to library; audio not yet acquired"); wire-serialized lowercase via `.value`. Later states arrive with the `acquire-track` spec.
