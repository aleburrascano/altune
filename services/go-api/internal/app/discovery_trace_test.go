...
func TestRequestPathProviderCallsAreTracedAndCountedOnce(t *testing.T) {
	...
	for _, route := range routes {
		t.Run(route.name, func(t *testing.T) {
			...
			disc := a.wireDiscovery(context.Background(), newClientFactory(rt))
...
// TestBackgroundChartCallsStayOffTheTrace pins the other half of the wrap
// decision: chart refresh runs under no request, so wrapping it would only cost
// the trace store memory for exchanges no correlation id can ever reach.
func TestBackgroundChartCallsStayOffTheTrace(t *testing.T) {
	rt := &countingProviderRT{}
	base := newClientFactory(rt)
	...
	if _, found := disc.requestStore.Get("corr-chart"); found {
		t.Error("a chart fetch reached the request trace store; background traffic must stay on the plain transport")
	}
}
