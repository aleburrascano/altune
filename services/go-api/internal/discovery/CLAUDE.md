...
- `service/` — `search.go` (the `Service` orchestrator), `merge.go` / `rank.go` / `diversity.go` (the Merge→Rank→reshape core), `enrich/` (detail-open enrichers), `eval/` (offline harness cores, including `library_corpus.go`'s frozen-corpus load/save and `parallel.go`'s `runParallel[T]` — the shared fan-out spine the five `Run*Eval` modes drive with their own worker closures), `album_normalize.go` (the pure album-slice helpers — dedup, the generic `sortByReleaseDateDesc[T]`, year-fill — split out of `get_artist_content.go`'s fetch orchestration), and `record_type.go` (the typed `RecordType` value with `Parse`/`Rank` — the single `record_type` taxonomy encoding that release-merge precedence and discography bucketing both read).
- `adapters/` — `providers/` (one file per provider, split by concern once a provider grows: SoundCloud spans `soundcloud.go` (adapter struct + `resolveAndFetch` retry), `soundcloud_search.go`, `soundcloud_content.go` and `soundcloud_types.go`; MusicBrainz keeps its search and DTOs in `musicbrainz.go`, its generic `mbMemo[V]` TTL cache in `mbcache.go`, and the release-group singleflight cluster in `musicbrainz_releasegroups.go`; MusicBrainz and Discogs share one ctx-aware `minIntervalLimiter{interval}.wait(ctx)` in `ratelimit.go` (both 1-req/sec, both honor ctx cancellation — iTunes' burst/GCRA limiter in `itunes.go` is a distinct beast and stays hand-rolled); the shared `isAuthStatus` predicate lives in `providerhttp.go`), `cache/`, `persistence/`, `handler/`, `catalogbridge/` (the read-only seam onto catalog for ownership stamping and track-number fill). In `cache/`, the unexported `redisJSON` (`redis_json.go`) holds the single `disabled()` nil-guard, the shared `"1"` negative sentinel, and the `getJSON[T]`/`setJSON` helpers embedded across every Redis cache — the result cache routes through the generic `RedisNameKeyedCache[T]`; artwork (confidence-gated write), identity (fan-out) and enrichment (non-hashed `kind:mbid` positive key) stay bespoke but reuse the guard. In `persistence/`, the generic `collectRows[T]` (`collect.go`) is the single `Next->Scan->append->Err` spine every read repo drives with its own scan closure; empty init is non-nil `[]T{}`, and a closure returns the `errSkipRow` sentinel to drop a malformed row (favorites' bad-kind rows) — everyone else surfaces a scan error as before.
