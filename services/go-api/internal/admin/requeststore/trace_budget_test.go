...
func TestTraceFields_CountTowardByteBudget(t *testing.T) {
	for name, record := range traceRecorders {
		t.Run(name, func(t *testing.T) {
			s := New()
			record(s, httputil.WithCorrelationID(t.Context(), "c1"), bigResults(50, 100))
			...
			record(s, httputil.WithCorrelationID(t.Context(), "c2"), bigResults(50, 100))
...
func TestTraceFields_EvictedRecordReleasesTraceBytes(t *testing.T) {
	s := New()
	s.maxRequests = 1
	s.RecordSearch(httputil.WithCorrelationID(t.Context(), "c1"), "q", nil, "u", nil, bigResults(10, 100))
	s.RecordContentFetch(httputil.WithCorrelationID(t.Context(), "c2"), ports.ContentFetchEvent{Kind: "albums"}, nil)
	assertWithinBudget(t, s)
