...
// logSafeError renders err for a log attribute with credentials and host
// filesystem layout removed. A raw acquisition error chain embeds subprocess
// stderr verbatim, and yt-dlp names its --cookies file there (ARCHITECTURE
// §2.7). failureReason already guards the stored/wire copy; this guards the log
// copy, mirroring httptrace's use of redact.Secrets for URL secrets.
func logSafeError(err error) string {
	if err == nil {
		return ""
	}
	return logSafeText(err.Error())
}

// logSafeText applies logSafeError's redaction to arbitrary diagnostic text,
// such as a recovered panic value. Token by token it masks the value of a
// --cookies flag, any path or file name naming a cookie jar, and any absolute
// filesystem path outside the OS temp dir (the per-job altune-acquire-* scratch
// dirs are the only layout worth keeping for triage). http(s) URLs are left
// intact apart from redact.Secrets' query-param scrub.
func logSafeText(s string) string {
	m := &pathMasker{tmp: filepath.Clean(os.TempDir())}
	s = quotedPathRe.ReplaceAllStringFunc(redact.Secrets(s), m.maskQuoted)
