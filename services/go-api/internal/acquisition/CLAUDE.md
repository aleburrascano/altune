...
- `service/` — `acquire.go` (`Execute`, `resolveIdentity`, `CoreSteps`, `failureReason`), `pipeline.go` (the `Step` chain), `steps_*.go`, `matching.go` (candidate scoring; named bucket comparators `lessResolved`/`lessTopic`/`lessOther`), `duration.go` (the duration-tolerance gate: the window functions plus the `lengthCorroborated`/`durationAcceptable` policy), `scheduler.go`, `joblog.go`, `retry_admission.go` (`RetryAdmission`, `ReacquireAdmission`, the shared `cooldownGate`; characterized by `retry_admission_test.go`). `Execute` acquires; `ExecuteReplace` swaps the audio of an already-ready track.
