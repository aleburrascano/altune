...
- `service/eval/` — the offline selection harness: `case.go` (golden format + embedded `goldens/`), `ports.go` (case-driven fakes), `harness.go` (`Run` over the real `CoreSteps`), `report.go` (per-class accuracy, baseline gating). Driven by `cmd/acquisitioneval`.
