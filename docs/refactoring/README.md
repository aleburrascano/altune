...
These are **on-demand reference docs**, living under `docs/` rather than `.claude/rules/` — a file in `.claude/rules/` without `paths:` frontmatter loads globally on every session, so on-demand catalogs must live outside that directory to carry zero standing token cost. The structural-review skills consult them by name via Read; they don't auto-load on every edit.
