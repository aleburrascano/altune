...
### Status (2026-07-23): shipped — MB-anchored verification

Built and **verified on the live API**. The title-cohesion layer (§7 above) was tried and **abandoned** — same-name artists share coincidental titles, so no title-overlap threshold separates them; ISRC was also ruled out (Deezer's artist endpoints don't expose it). The reachable robust signal is the artist's own **authoritative MB catalogue**: live-measured, the mis-bridged soul Che's Deezer shares **8%** of the rapper's MB release-groups; the real rapper Deezer shares **60%**.

`FilterGroupsByMBAnchor` (`release_verify.go`) verifies each id-fanout provider against the MBID's MB release-group titles and drops the ones that barely overlap (mis-bridged same-name artists). `v2Albums` is now **id-only** — the by-name completeness feed was itself a contamination + title-collision source (iTunes returning a different Che). Result on the bridged Che card: **89 → 25 items, zero soul-Che leakage**, the rapper's real discography with year/tracks/covers. Fail-open: no anchor / no MBID / MB error → keep all.

**Still open:** Spotify content returns nothing (fragile anon token / 429 — a dead strong source); top-tracks verification (the MB anchor is album-level); and finer record-type bucketing on a bridged card (id-only leaves only the surviving provider's coarser type). The permanent fix remains upstream — don't persist a cross-provider bridge whose catalogues don't overlap — so fractured identities are never stored.
