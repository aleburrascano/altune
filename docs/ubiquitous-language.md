...
- **Track** — a single audio recording with metadata (title, artist, optional album/year/genre/track_number/album_artist/isrc/duration/artwork_url, `audio_ref`) plus an `AcquisitionStatus`. Aggregate root of the **catalog** bounded context; identity is `TrackId` (UUID); owned by a `UserId`. Defined in `services/go-api/internal/catalog/domain/track.go`.
- **Playlist** — a user-created, named, ordered collection of tracks. Aggregate root of the **catalog** bounded context; identity is `PlaylistId` (UUID); owned by a `UserId`. Carries `name` (non-empty, ≤100 chars), timestamps, and an ordered tuple of `PlaylistTrack` entries — invariants: non-empty name, contiguous 0..N-1 positions, no duplicate track_ids. Defined in `services/go-api/internal/catalog/domain/playlist.go`.
- **PlaylistTrack** — value object for a track's membership in a playlist: `track_id: TrackId` + `position: int` (0-indexed). Lives inside the Playlist aggregate, not standalone. Defined in `services/go-api/internal/catalog/domain/playlist.go`.
- **AcquisitionStatus** — lifecycle of a saved track's audio acquisition: `pending` (not yet acquired), `ready` (acquired, streamable), `failed` (acquisition failed or file went missing; requires `failure_reason`). Wire-serialized lowercase. `audio_ref ↔ status` is a bidirectional invariant on Track: `audio_ref` set requires `ready`; `failed` requires `failure_reason` and clears `audio_ref`. Transitions are guarded and a disallowed one returns `ErrIllegalAcquisitionTransition` leaving the track unchanged: `MarkReady` from pending or failed, or from ready only under the same `audio_ref` (a different ref is refused); `ReplaceAudio` only from ready with audio; `MarkFailed` from pending or ready (audio went missing), not from failed; `FailAcquisition` (a failed acquisition attempt) only from pending; `RevertToPending` from ready or failed. Defined in `services/go-api/internal/catalog/domain/track.go`.
- **FailureCode** — the stable, machine-readable code that prefixes a failed track's `failure_reason` (`no_match_found`, `source_unavailable`, `download_failed`, `storage_failed`, `acquisition_cancelled`, `acquisition_failed`, `ytdlp_error`, `acquisition_interrupted`), optionally followed by `: ` and a human-readable detail such as a candidate-rejection summary. The acquisition side emits it; `FailureMessage` derives the wire `failure_message` from the code alone. `source_unavailable` is the one code that is evidence about the source rather than the track — it was throttled, unreachable, or not installed — so it must never be read as the track not existing. Defined in `services/go-api/internal/catalog/domain/failure.go`.
- **track IdempotencyKey** — optional client-minted token on `POST /v1/tracks` (the `Idempotency-Key` header), stable across retries of one logical save. Persisted on `Track` under a partial unique index `(user_id, idempotency_key)`; the create insert dedups on `ON CONFLICT DO NOTHING`, so two concurrent creates or a dropped-response retry sharing a key collapse to a single row (the key takes precedence over the content-derived dedup key). Nil when the client sends no key, falling back to content dedup. Mirrors the telemetry `event_id` pattern. Defined in `services/go-api/internal/catalog/domain/track.go`; sent by `apps/mobile/src/shared/api-client/tracks.ts`.
- **ResultKind** — discriminator on `SearchResult`: `artist`, `album`, `track`, or `playlist`. Wire-serialized lowercase. Defined in `services/go-api/internal/discovery/domain/enums.go`.
- **Confidence** — three-level enum on `SearchResult` for dedup-merge certainty: `high` (merged via an id-based `EntityResolutionTier` match — ISRC, UPC, MBID, or cross-provider bridge), `medium` (no id-based tier match, but merged from more than one source), `low` (standalone, unmerged result). Comparable: HIGH > MEDIUM > LOW. **Display-only since ADR-0007** — a client trust badge, not a ranking input; relevance (RRF fused with IDF-weighted token-similarity) drives order — there is no separate exact-match boost step. Defined in `services/go-api/internal/discovery/domain/enums.go` (type); assigned in `services/go-api/internal/discovery/service/merge.go`.
- **SourceRef** — one provider's reference to a merged `SearchResult`: `(provider, external_id, url)`. A multi-source result holds a tuple of these. Defined in `services/go-api/internal/discovery/domain/search_result.go`.
- **SearchResult** — the merged, dedup'd discovery result on the wire: `(kind, title, subtitle?, image_url?, confidence, sources: tuple[SourceRef,...], extras)`. Invariants: non-empty title, non-empty sources. Defined in `services/go-api/internal/discovery/domain/search_result.go`.
- **SearchQuery** — validated search input crossing into the use case: `(raw, query_norm, kinds: frozenset[ResultKind], limit)`. Invariants: non-empty `raw`, non-empty `kinds`, `1 ≤ limit ≤ 50`. Defined in `services/go-api/internal/discovery/domain/search_query.go`.
- **ProviderStatus** — per-provider outcome of one scatter-gather call: `ok`, `timeout`, `error`, `rate_limited`, `circuit_open`. Surfaced on the wire's `providers[]` array. Defined in `services/go-api/internal/discovery/domain/enums.go`.
- **CanonicalContentProvider** — the one provider (currently Deezer) whose ids back browseable content: album-track and artist-album lookups, featured-artist enrichment, the browseable-source ranking check, and the head of the artist-content fan-out. Tested via `IsCanonicalContentProvider`. Defined in `services/go-api/internal/discovery/domain/enums.go`.
- **Favorite** — a user's saved marker on an artist, album or track, used to lift matching Discovery search results. Identity is `(user_id, kind, key)` where `key` is the normalized name derived by `FavoriteKey` — artists key on the name alone, albums and tracks on `artist|title`. Favoriting an artist also lifts that artist's albums and tracks. Not an entity in its own right: albums and artists have no rows, so a Favorite names them rather than pointing at them. Defined in `services/go-api/internal/discovery/domain/favorite.go`.
- **SearchHistoryEntry** — per-user persisted search-history row. Identity `ID` (UUID); carries `(user_id, query, query_norm, executed_at, result_clicked_signature?)`. Defined in `services/go-api/internal/discovery/domain/records.go`.
- **TrackAddedToLibrary** — the `track_added_to_library` event type published on a track's own generic `events.Event` envelope (`id`, `type`, `payload`, `timestamp`) when a track is freshly saved to a library; a dedup hit publishes none. `track_id` and the track's other fields ride in `payload`. Defined in `services/go-api/internal/shared/events/types.go`; published from `services/go-api/internal/catalog/service/add_track.go`.
- **SearchPerformed** / **ResultClicked** — discovery domain events (past-tense, immutable, carry `occurred_at`). Logged in v1. Defined in `services/go-api/internal/discovery/domain/events.go`.
- **InteractionEvent** — the unified behavioral-telemetry envelope persisted to `discovery_events`: `(occurred_at, user_id, type, query_norm?, search_id?, payload)`. `EventType` is closed: `search_performed`, `results_shown`, `result_clicked`, `play`, `skip`, `completed`, `library_add`, `wrong_album`; new fields ride in JSONB `payload`. Defined in `services/go-api/internal/discovery/domain/events.go`.
...
- **SatisfactionConsumer** — the concrete `Name()` + `Signals(ctx, since)` behavioral-signal source, called directly (there is no `EventConsumer` port; a new signal is a new struct wired in by hand): `play`/`completed` (listen threshold 30s or 50% duration) ⇒ positive per-`result_signature` score; `skip` under 20s dwell ⇒ negative. Gated default-OFF by `BEHAVIORAL_RANKING_ENABLED`. Defined in `services/go-api/internal/discovery/service/behavioral_signals.go`.
- **BehavioralLabel** / **BehavioralCorpus** — a relevance label mined from a query→engagement chain: `completed`/`library_add` ⇒ positive, `wrong_album` ⇒ hard negative. `CorpusBuilder` materializes labels nightly into the eval-harness corpus format. Defined in `services/go-api/internal/discovery/service/eval/behavioral_corpus.go`.
...
- **ReplayCorpus** — the offline counterfactual scorer: scores a candidate ranking against `BehavioralCorpus` without serving it. Defined in `services/go-api/internal/discovery/service/eval/replay.go`.
...
- **EntityResolutionTier** — how two results were identified as the same entity during dedup: `mbid`, `isrc`, `upc`, `bridge` (cross-provider id match via MB url-relations), or `none`. Identifier-based only — no text/duration similarity. Defined in `services/go-api/internal/discovery/domain/enums.go`.

- **MBEnrichment** — immutable detail-screen enrichment from MusicBrainz: `mbid`, curated `genres`, `year`, `rating`+`rating_votes`, `primary_type`+`secondary_types` (album only), `external_ids`, HD `artwork_url`. Not persisted — live read on detail-open. Defined in `services/go-api/internal/discovery/domain/`.
...
- **EntityIdentity** — durable cross-provider identity record: `(provider, external_id, kind) → (mbid, xref)`. Lets a later MB-absent search resolve identity (and artwork) from what a prior MB-present search learned. Keyed on stable provider ids, never names. Persisted in the `entity_identity` table.
...
- **DiscogsArtistInfo** — artist enrichment resolved from Discogs: `(id, name, genre, country, overlap)`, `overlap` scoring how many of the artist's known albums a candidate Discogs artist's releases share. Not persisted. Defined in `services/go-api/internal/discovery/ports/ports_content.go`; resolved by `DiscogsAdapter.ResolveDiscogsArtist` in `services/go-api/internal/discovery/adapters/providers/discogs.go`.
- **DiscogsRelease** — one release from a Discogs artist's discography, `(title, year, type)`, used for the artist-overlap match above. Defined in `services/go-api/internal/discovery/ports/ports_content.go`.
...
- **QueuePosition** — a position-only save of the persisted queue snapshot: `(CurrentIdx, CurrentTrackId, PositionMs)`, no track lists. Applies only when the stored queue holds `CurrentTrackId` at `CurrentIdx`; ordered against full saves by the same stale-write guard. Sent via `PUT /v1/playback/queue-state/position`. Defined in `services/go-api/internal/playback/domain/queue_position.go`.
