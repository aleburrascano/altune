...
- **QueuePosition** — a position-only save of the persisted queue snapshot: `(CurrentIdx, CurrentTrackId, PositionMs)`, no track lists. Applies only when the stored queue holds `CurrentTrackId` at `CurrentIdx`; ordered against full saves by the same stale-write guard. Sent via `PUT /v1/playback/queue-state/position`. Defined in `services/go-api/internal/playback/domain/queue_state.go`.
