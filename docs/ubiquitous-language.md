...
- **FailureCode** — the stable, machine-readable code that prefixes a failed track's `failure_reason` (`no_match_found`, `download_failed`, `storage_failed`, `acquisition_cancelled`, `acquisition_failed`, `ytdlp_error`, `acquisition_interrupted`), optionally followed by `: ` and a human-readable detail such as a candidate-rejection summary. The acquisition side emits it; `FailureMessage` derives the wire `failure_message` from the code alone. Defined in `services/go-api/internal/catalog/domain/track.go`.
