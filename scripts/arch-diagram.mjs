// Render docs/architecture.md, the C4 map of the system in one file:
//   levels 1-2 (context, containers) from the hand-written LikeC4 model, via
//     `likec4 gen mermaid`; views are ordered by title, so title them "Level N: ...";
//   level 3 (modules) from graft's wiring graph: one dependency diagram per world,
//     a health summary, and a mutual-dependency (cycle) report.
// Run by `npm run arch` (after `graft build`) and, in --check mode, by
// `npm run arch:check` to fail on drift.
//
// Input:  docs/architecture/model.c4 (committed, edited by hand).
//         graft/.graph/wiring.json (structural graph; git-ignored, regenerable).
...
// Needs Node >= 22.22.3 for the pinned LikeC4.
...
function renderC4Views() {
  likec4("validate", C4_MODEL);
  const out = mkdtempSync(join(tmpdir(), "c4-"));
  try {
    likec4("gen", "mermaid", C4_MODEL, "-o", out);
