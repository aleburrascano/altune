...
- **Timers are scheduling, not state** — they live in a module-level `Map` keyed by track id outside the store; `start()` on a re-acquire cancels any lingering terminal sequence so a stale removal timer can't delete the restarted entry (`__tests__/downloadStore.test.ts` — pending rebuild per [test-taxonomy](../playbooks/test-taxonomy.md)).
