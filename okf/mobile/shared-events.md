...
## `acquisition_status` is validated at the SSE boundary, not asserted (2026-09-07)

`parseAddedTrack` guarded that `data.acquisition_status` was a *string* and then cast it `as TrackResponse['acquisition_status']` — a checked-then-widened cast that told the compiler the value was one of `pending | ready | failed` without ever confirming it. An off-contract status the socket could carry (`"queued"`, a not-yet-shipped lifecycle phase) passed the string guard and seeded every library, lookup and featuring cache — plus `trackStatusStore` — with a value outside the `AcquisitionStatus` union, which then flowed into `canPlay` and the row's phase rendering. `parseAddedTrack` now narrows through `asAcquisitionStatus`, a membership check against the three literals (the same shape the detail feature's `extras-accessors` already used for its untyped wire map); a non-member makes `parseAddedTrack` return `null`, which drops the event onto the existing thin-payload fallback (invalidate library-home + featuring) rather than caching a bad row. The parser is now the boundary that enforces the union instead of the cast that assumed it.

The mirror-image cast lived in `features/playback/hooks/useQueueResume.ts`: `saved.repeat_mode` arrives as a bare `string` from the queue-state wire type and was cast `as RepeatMode` before being checked `=== 'all' || === 'one'`. That one was already safe — the guard rejected anything off-contract — but it read as if the cast did the narrowing. It now goes through a local `asRepeatMode` membership check so both SSE-adjacent and resume-adjacent status parses are explicit narrowings, and neither claims a union it has not verified.
