...
`loadIndex` also **validates every entry's shape** before seeding it into store state (2026-09-07). The old guard checked only that the parsed JSON was a non-array object and then cast it straight to `Record<string, PinnedEntry>`, trusting each entry's `status`, `uri` and `version` — fields written by an arbitrarily old build and restored from device backups. That cast ran at store construction (`entries: loadIndex()`), so a persisted entry carrying a `status` this version does not recognise, or a `uri`/`version` of the wrong type, became live store state that `pinnedUri`, the download worker and the UI all read. `narrowEntry` now drops any value that is not an object with a string `trackId`, a known `PinnedStatus`, and (when present) a string `uri` and `version`; the rest of the index survives, and the drop count is logged (`console.warn`, since the slice is comment-free). This is the load-time counterpart to `reconcile`'s existing runtime drop of an unrecognised `status` when the file is gone — the two now agree that an unknown status is never trusted, one at construction and one per launch pass.
...
`saveIndex`'s write was a bare `catch {}` until 2026-09-07 and now `console.warn`s (`[offline] failed to persist pinned index; keeping in-memory only`) — the same in-memory-degrade-never-throw posture the telemetry outbox takes, since the on-disk index is only a cache of live store state; the log is the difference between a silent persistence failure and a traceable one.
...
## Why the event alone was not enough (2026-07-31)

Both halves of the section above hang off one live SSE event. If `track_acquisition_completed` never reaches the device, nothing else ever re-examines a cached file: `pinnedUri` answered from the index, `findPinned` matched `<trackId>.*`, `reconcile()` checked only that a file *exists*, and none of them had any notion of *which* audio the file held. A re-acquisition that completed while the app was backgrounded, offline, or simply not running therefore left the old recording authoritative on that device **permanently** — with the admin panel, the resolved source URL and the bucket all correctly showing the new one. Re-acquiring from the admin panel, the normal way to use that button, is precisely the case where the app is not in the foreground.
...
- `expectedVersion === undefined` — the caller could not resolve URLs at all. An offline load must still play the pinned file; refusing here would break the feature's entire promise to protect against a bug that only manifests online.
- `expectedVersion === ''` — the *server* has no version, i.e. a track not acquired since the column landed. Treating empty as a mismatch would re-download every pre-existing pinned track on first play.
- a version match — the ordinary path.

`reconcile()` carries the recorded version forward (`{ ...entry, status: 'ready', uri }`) rather than rebuilding the entry from scratch. Dropping it there would silently mean *every* pinned track mismatches on the launch after any reconcile pass and re-downloads the whole offline library.

The accepted limit: a copy that is *already* stale on a device when this ships is not repaired by it. Its entry has no version and the server reports the version from the last acquisition, so the mismatch heals it on the next play — but a track whose stale copy predates the migration and is never re-acquired again keeps serving old audio until the user unpins it. A one-time repair pass (drop every cached file with no recorded version) was considered and rejected as not worth re-downloading a whole offline library for a bug that only bit tracks the user had re-acquired.
