...
A cache write failure must never fail the search — every write is best-effort. The enrichment cache distinguishes a hit from a miss (never enriched, or the entity carries no ids) rather than conflating a miss with an empty enrichment, and the name-keyed cache checks only that the `Get` succeeded, never its content. `ArtworkCache.Set` carries the confidence overwrite guard: a weaker result never replaces a real, higher-confidence image.
