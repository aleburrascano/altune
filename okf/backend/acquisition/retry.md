...
`RetryHandler` (`adapters/handler/retry_handler.go`) is a thin translation layer: retries carry no source URL — the request is by `trackId`, so acquisition re-searches by metadata; it loads the track, calls `admission.Admit`, and maps `ErrRetryNotFailed` → 409 Conflict and `ErrCooldownActive` → 429 Too Many Requests. On admission it calls `scheduler.Schedule(userId, trackId, "")` — retries carry no source URL (the request is keyed by `trackId`, not a pasted link), so acquisition always falls back to the full search pipeline (see [pipeline](pipeline.md)) rather than retrying a specific candidate URL. The `*service.RetryAdmission` is constructed once at the composition root and injected through `NewRetryHandler`, so admission state is owned by the composition root rather than the handler — there is exactly one retry entry point in the app today, and its gate is now wired explicitly like every other dependency (see [app-wiring](../app-wiring.md)).
...
`RetryAdmission` is unchanged: `AcquisitionFailed` only (`ErrRetryNotFailed` → 409), one per track per 60s. `ReacquireAdmission` is its mirror for the opposite case — it requires `track.IsStreamable()`, i.e. ready *with* an audio ref (`ErrReacquireNotReady` → 409), because replacing audio that does not exist is retry's job. Both surface `ErrCooldownActive` → 429. Each holds its own gate, so a retry and a re-acquire on the same track do not share a cooldown; they are different operations with different risk, and rate-limiting them together would be arbitrary.
