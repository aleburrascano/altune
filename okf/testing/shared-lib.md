...
**Two query keys added (2026-08-02), covered by the existing key tests.** `libraryKeys.tracksAll` and `discoveryKeys.favorites`. The one claim worth a row that the current suite does not make is the collision boundary: `tracksAll(q, sort)` and `tracks('all', sort)` share a three-element prefix and are distinguished only by length, which is the kind of thing a future refactor of the factory can break without any type error. Coverage and mutation figures are unchanged — both additions are literal tuples.
