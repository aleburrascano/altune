...
`PgxEventStore` (`event_repo.go`) implements four ports (`EventStore`, `EventQuery`, `BehavioralSignalStore`, `BehavioralLabelStore`) over this one table (see [telemetry](../backend/discovery/telemetry.md)): `Append` (the insert above), `ZeroResultQueries`/`NonZeroNoClickQueries` (coverage-gap SQL over `payload->>'zero_result'`), `SatisfactionSignals` (nets `play`/`completed` (+1) against short-dwell `skip` (−1) per `result_signature`, threshold `shortDwellThresholdMs = 20000`), and `BehavioralLabels` (joins engagement events back to their `search_performed` via `search_id` to mine free relevance labels, `wrong_album` ⇒ Polarity −1 else +1). The `SessionSignalStore` port (which `AbandonedSearches` — no-click search reformulated within 60s, joined via `payload->>'session_id'` — used to satisfy) was removed, but `AbandonedSearches` itself is not dead: it was folded into the `EventQuery` interface (`ports/ports_telemetry.go`) and is actively called from `CoverageSignalAService.Execute` (`coverage_signal_a.go`) alongside `ZeroResultQueries`/`NonZeroNoClickQueries` (see [eval-harness](../backend/discovery/eval-harness.md)).
