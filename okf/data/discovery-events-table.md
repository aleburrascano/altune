...
A malformed JSON payload is skipped for that row rather than raising a `22P02` that would brick the whole batch. `event_id` is NULL for fire-and-forget events so they never collide on the idempotency key; only label-critical events carry one. The behavioral-signal read is read-only and never on the write path.

The migration files carry no SQL comments — the backend is comment-free — so the schema's rationale lives in this concept doc, not inline in `migrations/`.
