...
Two tables in `001_baseline.sql` back the `Playlist` aggregate (see [[catalog]]). `playlists`: `id UUID PK`, `user_id UUID NOT NULL`, `name TEXT NOT NULL`, `created_at`/`updated_at TIMESTAMPTZ NOT NULL DEFAULT now()`. `playlist_tracks` is the membership join table: `playlist_id UUID NOT NULL REFERENCES playlists(id) ON DELETE CASCADE`, `track_id UUID NOT NULL REFERENCES tracks(id) ON DELETE CASCADE` (see [[tracks-table]]), `position INTEGER NOT NULL`, with composite `PRIMARY KEY (playlist_id, track_id)` (a track cannot appear twice).
