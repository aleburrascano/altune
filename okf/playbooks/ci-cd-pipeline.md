...
**test-backend.yml** — the baseline gate. Triggers on PRs and pushes to `main` touching `services/go-api/**`. Runs `go vet ./...`, then depguard via `golangci/golangci-lint-action` (pinned v2.3.0; rules in `services/go-api/.golangci.yml` — domain-purity and shared-never-imports-features, per the structure-audit F1 enforcement), then `go test -race -count=1 ./...` with `CGO_ENABLED=1` on an ubuntu-latest runner (the race detector needs cgo, which the dev box — Windows, no C toolchain — can't provide locally; this is where data races in concurrent telemetry code, e.g. the log ring, event tap, provider store, scheduler counters, alert monitor, are actually caught).
