...
Alert mirroring is deliberately out of v1: go-api's alert monitor only pushes to ntfy and holds no
readable state (`internal/admin/alert/monitor.go`), so there is nothing to read. It is a follow-up
once go-api grows a readable alerts endpoint.
