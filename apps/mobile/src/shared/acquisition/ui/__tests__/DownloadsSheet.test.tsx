...
describe('DownloadsSheet', () => {
  ...
  it('shows the number of items in progress', () => {
    const items = [entry({ trackId: asTrackId('t1') }), entry({ trackId: asTrackId('t2') })];
  ...
  it("renders a row's phase from the live store rather than the item prop", () => {
    act(() => {
      useDownloadStore.getState().start(asTrackId('t1'), { title: 'Live track' });
      useDownloadStore.getState().progress(asTrackId('t1'), 'downloading');
    });
    const items = [entry({ trackId: asTrackId('t1'), phase: 'done', title: 'Live track' })];
  ...
  it('updates the rendered phase when the store changes, without new props', () => {
    ...
    const items = [entry({ trackId: asTrackId('t1'), title: 'Live track' })];
    render(<DownloadsSheet visible items={items} onClose={jest.fn()} />);
    expect(screen.getByText('Downloading…')).toBeTruthy();

    act(() => {
      useDownloadStore.getState().progress(asTrackId('t1'), 'finishing');
    });
  ...
  it('falls back to a working label when the store has no entry for the track', () => {
    const items = [entry({ trackId: asTrackId('unknown'), phase: 'finding' })];
  ...
  it('falls back to generic title and blank artist when the entry has none yet', () => {
    const items = [entry({ trackId: asTrackId('t1'), title: null, artist: null })];
