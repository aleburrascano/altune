...
// Remote kill switches for the app's background loops (#955). Each loop reads its switch before it
// starts and before each further unit of work, and subscribes to be told when it flips. The switch
// document is the same shape the server-reported prefetch switch uses (#824): a boolean per loop,
// the last document seen wins, and a key the document does not send leaves that loop enabled. The
// last document applied is persisted, so a loop switched off stays off from the next cold start,
// before any refresh has returned.

export type KillSwitchLoop = 'serverEvents' | 'telemetryFlush' | 'offlineDownloads';
