...
- `navigation.ts` — `openDetail`. `hooks/` — `useSaveTrack`, `useLateralNav`, `useAlbumTracks`, `useArtistContent`, `useDetailEnrichments`, `useEnrichResult`, `useOwnedTrack`, `useOwnedPlayback` (the shared owned-playback tail: `owned`/`playButton`/`onPlayOwned`/`saveStateFor`/`onQuickSave`, consumed by both detail-state hooks), `useAlbumDetailState`, `useArtistDetailState`.
