...
- **Artist content fetch (multi-provider + SoundCloud)** — `useArtistContent` hook accepts the full `sources` array + `artistName`. Top tracks use the best single source (Deezer > Last.fm > MusicBrainz). Albums fan out to **one source per provider** (deduplicated — merged search results can carry multiple sources from the same provider for different same-name artists) in parallel via `Promise.allSettled`, PLUS a separate SoundCloud query using the artist name (the SC adapter resolves the name to a username internally). All results merged/deduped by lowercased title (keep highest `track_count`). Albums sorted by `release_date` or `year` descending.
