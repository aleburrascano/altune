...
Use `db:"column_name"` tags for sqlx, `pgx.CollectRows` with `pgx.RowToStructByName` for pgx. Handle NULLable columns with pointer fields (`*string`, `*time.Time`) — they work cleanly with both scanning and JSON marshaling.
...
For transaction patterns, isolation levels, `SELECT FOR UPDATE`, and locking variants, see library documentation.
