...
Dispatch one read-only `Explore` agent per bounded context under `services/go-api/internal/` (skip `app/`; an argument narrows to a single context). Each agent reads [HEURISTICS.md](HEURISTICS.md) and returns a findings inventory — every shallow module, needless coupling, duplication, and dead path it finds, _relentless_, nothing held back. Every finding carries a **blast radius**: the callers that ripple if it changes, verified by reference search across packages — never eyeballed from one file. Cross-package usage is invisible to a single read; a function that looks dead in `app.go` may be called from `search_wiring.go`.
